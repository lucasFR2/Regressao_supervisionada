...
class StandardScalerScratch:
    ...
    def fit(self, X):
        # X é uma lista de listas (matriz de características)
        n_features = len(X[0])
        n_samples = len(X)
        self.means = []
        self.stds = []
        ...
        for j in range(n_features):
            ...
            # Desvio padrão populacional com tolerância para evitar divisão por zero
...
class OneHotEncoderScratch:
    ...
    def __init__(self):
        self.categories_map = {} # mapeia coluna para lista de categorias únicas (excluindo a primeira)
        self.cat_cols = []
        self.num_cols = []
        self.feature_names = []
    ...
    def fit(self, df_data, cat_cols, num_cols):
        ...
        # Guardar as categorias únicas para cada coluna categórica
        for col in cat_cols:
            unique_vals = sorted(list(set(row[col] for row in df_data)))
            # Drop first para evitar multicolinearidade (como drop_first=True no pandas)
            if len(unique_vals) > 1:
                self.categories_map[col] = unique_vals[1:]
            else:
                self.categories_map[col] = unique_vals
                
        # Construir nomes das variáveis finais
    ...
    def transform(self, df_data):
        ...
        for row in df_data:
            new_row = []
            # Primeiro adicionar numéricos
            for col in self.num_cols:
                new_row.append(float(row[col]))
            # Adicionar codificações das categóricas
...
def load_and_preprocess_scratch(file_path):
    ...
    # A partir do dicionário de atributos do dataset:
...
class Node:
    ...
    def __init__(self, feature=None, threshold=None, left=None, right=None, *, value=None):
        self.feature = feature       # índice da característica para divisão
        self.threshold = threshold   # limite de divisão
        self.left = left             # subárvore esquerda
        self.right = right           # subárvore direita
        self.value = value           # valor se for folha
...
class DecisionTreeRegressorScratch:
    ...
    def __init__(self, max_depth=5, min_samples_split=5, max_features=None):
        ...
        self.max_features = max_features # número de variáveis a considerar a cada split
        self.root = None
    ...
    def _best_split(self, X, y, feature_indices):
        ...
        for idx in feature_indices:
            # Obter todos os valores dessa característica
            values = [row[idx] for row in X]
            unique_vals = sorted(list(set(values)))
            
            # Se só tiver um valor único, não há como dividir
            if len(unique_vals) <= 1:
                continue
                
            # Testar limites de divisão (midpoints entre valores adjacentes)
            # Para acelerar em datasets grandes, podemos usar um subconjunto de thresholds (ex: quantis)
            thresholds = []
            if len(unique_vals) > 10:
                # Escolher 10 percentis uniformes para testar
    ...
    def _build_tree(self, X, y, depth=0):
        ...
        # Determinar variáveis a serem avaliadas (bagging de features se max_features estiver setado)
        ...
        # Achar o melhor split
        best_idx, best_thresh = self._best_split(X, y, feature_indices)
        
        # Se nenhuma divisão for encontrada, retornar nó folha
        if best_idx is None:
            leaf_value = sum(y) / len(y) if y else 0.0
            return Node(value=leaf_value)
...
class RandomForestRegressorScratch:
    ...
    def fit(self, X, y):
        ...
        # Configurar padrão de variáveis a testar se não especificado (D/3 é padrão para regressão)
    ...
    def predict(self, X):
        # Fazer a predição acumulando a saída de todas as árvores
        predictions = []
        n_samples = len(X)
        
        # Matriz de predições: cada linha é uma amostra, cada coluna é uma árvore
        tree_preds = [tree.predict(X) for tree in self.trees]
        
        # Média das predições para cada amostra
        for i in range(n_samples):
    ...
    def _accumulate_importance(self, node, importance):
        if node is None or node.is_leaf():
            return
        # Acumular um ponto por nó de divisão no índice da variável
