...
def compute_histogram(data, bins=10):
    ...
    # Se os valores forem constantes
    if min_val == max_val:
...
def compute_correlation_matrix(X, col_names):
    ...
    # Calcular médias e desvios padrões para cada coluna
    means = []
    stds = []
...
def main():
    ...
    for subject, file_path in datasets.items():
        ...
        # Encontrar correlação das variáveis numéricas originais
        # Nota: as variáveis numéricas estão nas primeiras posições de X
        ...
        # 3. Divisão de treino e teste (70/30)
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42)
        
        # 4. Normalização das variáveis numéricas (apenas as colunas numéricas de X)
        scaler = StandardScalerScratch()
        # Selecionar sub-matrizes numéricas para ajustar o scaler
        ...
        # Substituir os valores normalizados de volta em X_train e X_test
        X_train_scaled = []
        ...
        # Amostras para os gráficos de real vs previsto e resíduos (limitar a 80 pontos para o gráfico ficar legível no PDF)
        ...
        # Guardar todas as informações no dicionário principal
        all_results[subject] = {
            "sample_counts": {
                "total": len(y),
                "train": len(y_train),
